...
class Gui:
    def __init__(
        ...
        """Start GUI program."""
        self.SOFTWARE_ICON = "icofile.ico"
        ...
        tk.Label(
            self.root, text=__program_name__, bg="white", font="Arial 16 bold"
        ).grid(row=0, pady=(55, 0))
        tk.Label(
            self.root,
            text="Individual close contact restrictions\nfor nuclear medicine patients",
            bg="white",
            font="Arial 10",
        ).grid(row=1, padx=(121, 121), pady=(0, 0))
        ...
        options = tk.Menu(mainmenu, tearoff=0)
        mainmenu.add_cascade(label="Options", menu=options)
        options.add_command(label="Organisation", command=self.options_organisation)
        options.add_command(label="Initial Values", command=self.options_init_vals)
                                    ...
                                    (
                                        ...
                                        collections.OrderedDict(
                                            [
                                                ...
                                                (
                                                    "c",
                                                    np.log(2) / generic_parameters[2],
                                                ),
                                                (
                                                    "d",
                                                    np.log(2) / generic_parameters[3],
                                                ),
                                            ]
                                        ),
                                    ),
                                    (
                                        ...
                                        collections.OrderedDict(
                                            [
                                                ...
                                                ("fraction_1", generic_parameters[1]),
                                                ("half_life_1", generic_parameters[2]),
                                                ("half_life_2", generic_parameters[3]),
                                            ]
                                        ),
                                    ),
                                ]
    ...
    def set_default_settings(self):
        self.email = "RadiationSafety@sa.gov.au"
        self.url = "sahealth.sa.gov.au/sami"
        self.report_logo = os.path.abspath("report_logo.png")
        # the following would let you call the script from a different dir, but won't work for cx_Freeze exe
        # self.report_logo = os.path.join(os.path.dirname(__file__), self.report_logo)
        ...
        site_options = [
            ...
            self.site_dict(
                ...
                "120 - 130 Haydown Road",
                "Elizabeth Vale SA 5112",
                "(08) 8182 9999",
            ),
            self.site_dict(
                "The Royal Adelaide Hospital",
                "Level 2, Lift E - 1 Port Road",
                "Adelaide SA 5000",
                "1300 724 319",
            ),
            self.site_dict(
                "Women's and Children's Hospital",
                "Level 2, Rogerson and Queen Victoria Buildings, 72 King William Road",
                "North Adelaide SA 5006",
                "(08) 8161 6055",
            ),
            self.site_dict(
                "Flinders Medical Centre",
                "Level 2, Medical Imaging, Flinders Drive",
        ...
        self.init_vals = {
            ...
            "detector_calibration_factor": 0.75,
            "curve_fit_model": "biexponential",
            "data_directory": os.getcwd(),
            "patient_handout_directory": os.getcwd(),
            "summary_report_directory": os.getcwd(),
            "patient_handout_comments": "For the next two days, continue to take precautions with your urine and saliva. Sit down while urinating, shut the toilet lid before flushing, and flush twice. Avoid contacting other people with your saliva and wash your toothbrush thoroughly after each use.",
        }
    ...
    def read_sites(settings_odict):
        ...
        for site in settings_odict["root"]["organisation"]["site"]:
            if any(x not in site for x in site_keys):
                raise KeyError(
                    "There is a site is missing one or more of the keys: {}".format(
                        ", ".join(site_keys)
                    )
                )
    ...
    def read_init_vals(self, settings_odict):
        ...
        if any(x not in init_vals for x in self.init_vals):
            raise KeyError(
                'File "{}" is missing one of the following keys: {}'.format(
                    self.FILENAME_SETTINGS, self.init_vals.keys()
                )
            )
    ...
    def options_organisation(self):
        ...
        tk.Button(
            window, text="Submit", command=lambda: submit_options_organisation(self)
    ...
    def options_init_vals(self):
        ...
        patient_handout_directory_entry[
            "xscrollcommand"
        ...
        summary_report_directory_entry[
            "xscrollcommand"
        ...
        tk.Button(
            window, text="Submit", command=lambda: submit_options_init_vals(self)
    ...
    def load_patient(self):
        ...
        filepath = filedialog.askopenfilename(
            initialdir=self.init_vals["data_directory"]
        ...
        if filepath != "" and filepath != ():
            ...
            if int(self.odict["data"]["patient_finished"]):
                ...
                if self.odict["data"]["patient_finished_by"] != "0":
                    self.viewing_completed_patient_label[
                        "text"
    ...
    def patient_details(self):
        ...
        def retrieve_patient_details(self):
            ...
            try:
                ...
                return
            if e_num_treatments_in_year < 0.0:
                messagebox.showerror(
                    "Error", "Bad number of treatments in a year", parent=window
                )
                return
        ...
        if "pregnancy_excluded" in self.odict["data"]["patient_details"]:
            hygiene = int(self.odict["data"]["patient_details"]["pregnancy_excluded"])
        else:
            hygiene = None
        if "pregnancy_excluded" in self.odict["data"]["patient_details"]:
            breastfeeding = int(
        ...
        if dob is not None:
            ...
            dob_year_entry.insert(0, dob.year)
    ...
    def administration_details(self):
        ...
        def retrieve_calibration(self):
            ...
            if (
                ...
                if r_calib_datetime > str2datetime(
                    ...
                    messagebox.showerror(
                        "Error", "Calibrated after administration", parent=window
                    )
                    return
            ...
            if r_admin_activity is not None:
                if r_admin_activity <= 0.0:
                    messagebox.showerror(
                        "Error",
                        "Administered activity less than or equal to zero",
                        parent=window,
                    )
                    return
            ...
            if not admin_day_entry.get():
                admin_day_entry.insert(0, r_calib_datetime.day)
            if not admin_month_entry.get():
                admin_month_entry.insert(0, r_calib_datetime.month)
            if not admin_year_entry.get():
                admin_year_entry.insert(0, r_calib_datetime.year)

            if not resid_day_entry.get():
                resid_day_entry.insert(0, r_calib_datetime.day)
            if not resid_month_entry.get():
                resid_month_entry.insert(0, r_calib_datetime.month)
            if not resid_year_entry.get():
                resid_year_entry.insert(0, r_calib_datetime.year)
        ...
        def retrieve_administration(self):
            ...
            if "calibration_datetime" in self.odict["data"]["administration_details"]:
                if r_admin_datetime < str2datetime(
                    ...
                    messagebox.showerror(
                        "Error", "Administered before calibration", parent=window
                    )
                    return
            ...
            if r_resid_a != "":
                ...
                if r_resid_datetime < r_admin_datetime:
                    messagebox.showerror(
                        "Error",
                        "Residual measured before administration",
                        parent=window,
                    )
                    return
            ...
            r_admin_activity = compute_administered_activity(
                self, c_admin_datetime=r_admin_datetime, c_residual=r_residual
            )
            if r_admin_activity is not None:
                if r_admin_activity <= 0.0:
                    messagebox.showerror(
        ...
        if calib_datetime is not None:
            ...
            calib_year_entry.insert(0, calib_datetime.year)
        ...
        if admin_datetime is not None:
            ...
            admin_year_entry.insert(0, admin_datetime.year)
        ...
        if resid_datetime is not None:
            ...
            resid_year_entry.insert(0, resid_datetime.year)
    ...
    def clearance_data(self):
        ...
        def retrieve_clearance_data(self):
            try:
                ...
                return
            if r_detector_calibration_factor <= 0.0:
                messagebox.showerror(
                    "Error", "Bad detector calibration factor", parent=window
                )
                return
        ...
        def retrieve_fit_choice(self):
            ...
            hrs_elapsed = [
                child1["hours_elapsed"]
                for child1 in self.odict["data"]["clearance_data"]["measurement"]
                if not int(child1["exclude"])
            ]
            dr_xm = [
                child1["doserate_corrected"]
                for child1 in self.odict["data"]["clearance_data"]["measurement"]
                if not int(child1["exclude"])
            ]
                    ...
                    self.odict["data"]["clearance_data"][
                        ...
                        [
                            ...
                            (
                                ...
                                collections.OrderedDict(
                                    [
                                        ("dose_rate_xm_init", meaningful_params_exp[0]),
                                        (
                                            "effective_half_life",
        ...
        for x in range(self.N_MEASUREMENTS_MAX):
            ...
            if prev_datetime is not None:
                ...
                year[x].insert(0, prev_datetime.year)
                ...
                year[x].insert(0, admin_datetime.year)
    ...
    def patient_discharge(self):
        ...
        if "actual_datetime" in self.odict["data"]["patient_discharge"]:
            ...
            discharge_year_entry.insert(0, discharge_datetime.year)
        else:
            admin_datetime = str2datetime(
                self.odict["data"]["administration_details"]["administration_datetime"]
            )
            discharge_month_entry.insert(0, admin_datetime.month)
            discharge_year_entry.insert(0, admin_datetime.year)
    ...
    def generate_reports(self):
        def get_patient_handout(self):
            ...
            instructions_1 = (
                "After you have been given the radioactive substance to treat your medical condition, "
                + "you will be a potential source of radiation exposure to other persons around you, "
                + "particularly if they approach closer than one metre. You will have already been given some "
                + "quite detailed instructions on the general precautions which you should take in order to "
                + "minimise the radiation contamination and exposure to other persons. Specific instructions "
                + "which relate to "
            )
            ...
            r.font.bold = True
            r = p.add_run()
            r.text = instructions_3
            r = p.add_run()
            r.text = instructions_4
            r.font.bold = True
            r = p.add_run()
            r.text = instructions_5
            ...
            col2 = ""
            col3 = ""
            if df["applies"].dtype == "int64":
                n_rows = df["applies"].value_counts()[1]
            else:
                n_rows = df["applies"].value_counts()["1"]
            ii = 1
            for index, row in df.iterrows():
                if int(row["applies"]):
                    ...
                    if discharge_datetime_str is not None:
                        if datetime_end < discharge_datetime:
                            col3 += "Yes"
                    col3 += "\n" * n_new_lines
        ...
        def get_summary_report(self):
            ...
            if generic:
                ...
                generic_str = (
                    "The calculated close-contact restrictions below are based on actual clearance and exposure "
            ...
            instructions_2 = (
                ...
                + "instructions which outline the procedures to be followed and the duration of time for which "
                + "specific restrictions will apply."
            )
